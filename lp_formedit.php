...
include('config.php');
$id = $_GET['id'];
$query = mysqli_query($connect, "SELECT id, judul, tempat, kategori, subkategori, deskripsi, foto, waktutemu FROM laporan_penemuan WHERE id = $id");
while ($edit_lp = mysqli_fetch_array($query)){
	$id_lp = $edit_lp['id'];
	$judul = $edit_lp['judul'];
	$tempat = $edit_lp['tempat'];
	$kategori = $edit_lp['kategori'];
	$subkategori = $edit_lp['subkategori'];
	$deskripsi = $edit_lp['deskripsi'];
	$foto = $edit_lp['foto'];
	$waktutemu = $edit_lp['waktutemu'];
}
?>
...
<body>
	...
	<form action="lp_prosesedit.php" method="POST" enctype="multipart/form-data">
		<table>
				<input type ="hidden" name="id_lp" value="<?php echo $id_lp;?>"/></td>
			<tr>
				<td>Judul</td>
				<td>:</td>
			...
			<tr>
				<td>Tempat</td>
				<td>:</td>
				<td><input type="varchar" name="tempat" placeholder="Pojok MIPA" value="<?php echo $tempat;?>" required autofocus></td>
			</tr>
			<tr>
				<td>Kategori</td>
				<td>:</td>
				...
				if ($kategori == "perlengkapan_kuliah") {
					...
					echo "<option value='perlengkapan_kuliah'>Perlengkapan Kuliah</option>";
				}
				if ($kategori == "lainnya") {
			...
			<tr>
				<td>Waktu Ditemukan</td>
				<td>:</td>
				<td><input type="datetime-local" name="waktutemu" required/></td>
			</tr>
			<tr>
				<td>Foto</td>
				<td>:</td>
